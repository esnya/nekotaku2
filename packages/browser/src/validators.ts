...
function validateString(value: any, validator: (value: string) => boolean | string): boolean | string {
...
export function maxLength(n: number): Validator {
  return (value) => {
    return validateString(value, (str) => {
      if (value.length > n) return `${n}文字以下で入力してください`;
      return true;
    });
  };
}
...
export function minLength(n: number): Validator {
  return (value) => {
    return validateString(value, (str) => {
      if (value.length < n) return `${n}文字以上入力してください`;
      return true;
    });
  };
}
